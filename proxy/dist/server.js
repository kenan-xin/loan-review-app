...
function pruneOldDumps(context) {
    try {
...
async function logSseFrames(stream, ctx) {
...
app.post("/api/loan-review", upload.single("ca"), async (req, res) => {
    ...
    try {
        ...
        const dumpPath = path.join(DUMP_DIR, `sse-${new Date().toISOString().replace(/[:.]/g, "-")}-${originalName.replace(/[^\w.-]/g, "_")}.jsonl`);
        logger.info(context, "Dumping SSE events", { dumpPath });
        const [logBranch, clientBranch] = response.body.tee();
        logSseFrames(logBranch, { context, startedAt: t0, file: originalName, dumpPath }).catch((err) => logger.error(context, "log-drain crashed", { error: String(err) }));
